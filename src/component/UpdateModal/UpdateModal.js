...
function UpdateModal({ show, handleClose,props, getFavMovie,
    titleInput,
    setTitleInput,
    imageInput,
    setImageInput,
    ...
    const update = async (id) => {
        ...
        let updated = { title: titleInput, release_date: props.release_date, overview: props.overview, poster_path: props.poster_path, image:imageInput, comment:commentInput }

        await axios.put(`https://movieasma.herokuapp.com/updateMovie/${id}`, updated)
            .then(() => {
                getFavMovie();
            }).catch(err => {
                console.log(err);
