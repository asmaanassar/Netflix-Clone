...
function FavList() {
    ...
    const getFavMovie = async () => {
        ...
        return await axios.get(`https://movieasma.herokuapp.com/favorit`)////fixed it
            .then(result => {
    ...
    const deleteFromFav = async (id) => {
        await axios.delete(`https://movieasma.herokuapp.com/deleteMovie/${id}`)
            .then(() => {
                console.log("deleted :(")
                getFavMovie();
            }).catch((err) => {
                        ...
                        data.length && data.map((ele) => (
                            <Col key={ele.id} md={4}>
                                <Card className='div-card'>
                                    <Card.Img className='div-card-img' variant="top" src={`https://image.tmdb.org/t/p/w500${ele.data.poster_path}`} />
                                    <Card.Body>
